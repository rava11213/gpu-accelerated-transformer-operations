...
struct Options { std::string op = "all", variant = "optimized"; int m = 512, n = 512, k = 512, rows = 1024,
  cols = 768, warmup = 10, iterations = 100; bool check = false, csv = false; };
...
Options parse(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (a == "--variant") o.variant = value(); else if (a == "--check") o.check = true;
    else if (a == "--csv") o.csv = true; else if (a == "--help") {
      std::cout << "Usage: transformer_ops [--op all|matmul|softmax|layernorm|residual-layernorm]\n"
                   "  [--variant optimized|naive] [--m N --n N --k N] [--rows N --cols N]\n"
                   "  [--warmup N --iterations N] [--check] [--csv]\n"; std::exit(0);
  ...
  if (o.op != "all" && o.op != "matmul" && o.op != "softmax" && o.op != "layernorm" && o.op != "residual-layernorm")
    throw std::runtime_error("invalid --op");
  if (o.variant != "optimized" && o.variant != "naive") throw std::runtime_error("--variant must be optimized or naive");
  return o;
}
...
void report(const std::string& op, const Options& o, float gpu, double cpu, float error) {
  if (o.csv) {
    const std::string reported_variant = op == "matmul" ? o.variant : "optimized";
    std::cout << op << ',' << reported_variant << ',' << o.m << ',' << o.n << ',' << o.k << ','
              << o.rows << ',' << o.cols << ',' << std::fixed << std::setprecision(6) << gpu << ',';
    if (o.check) std::cout << cpu << ',' << std::scientific << error;
    else std::cout << "NA,NA";
    std::cout << '\n'; return;
  }
  std::cout << std::left << std::setw(12) << op << " GPU " << std::fixed << std::setprecision(4) << gpu
            << " ms";
  if (o.check) std::cout << " | CPU " << cpu << " ms | speedup " << cpu / gpu
...
void run_matmul(const Options& o) {
  ...
  auto launch = [&]{ if (o.variant == "naive") transformer_ops::launch_matmul_naive(da, db, dc, o.m, o.n, o.k);
                     else transformer_ops::launch_matmul(da, db, dc, o.m, o.n, o.k); };
  ...
  report("matmul", o, ms, cpu, err); CUDA_CHECK(cudaFree(da)); CUDA_CHECK(cudaFree(db)); CUDA_CHECK(cudaFree(dc));
}
...
void run_rowwise(const Options& o, bool layernorm) {
  ...
  report(layernorm?"layernorm":"softmax",o,ms,cpu,err); CUDA_CHECK(cudaFree(dx)); CUDA_CHECK(cudaFree(dy)); if(layernorm){CUDA_CHECK(cudaFree(dg));CUDA_CHECK(cudaFree(db));}
}

void run_residual_layernorm(const Options& o) {
  auto x=random_vector(size_t(o.rows)*o.cols,6), residual=random_vector(x.size(),7);
  auto gamma=random_vector(o.cols,8), beta=random_vector(o.cols,9);
  std::vector<float> out(x.size()), residual_out(x.size()), ref(x.size());
  float *dx,*dr,*dg,*db,*dy,*dro;
  CUDA_CHECK(cudaMalloc(&dx,x.size()*sizeof(float))); CUDA_CHECK(cudaMalloc(&dr,x.size()*sizeof(float)));
  CUDA_CHECK(cudaMalloc(&dg,gamma.size()*sizeof(float))); CUDA_CHECK(cudaMalloc(&db,beta.size()*sizeof(float)));
  CUDA_CHECK(cudaMalloc(&dy,x.size()*sizeof(float))); CUDA_CHECK(cudaMalloc(&dro,x.size()*sizeof(float)));
  CUDA_CHECK(cudaMemcpy(dx,x.data(),x.size()*sizeof(float),cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(dr,residual.data(),residual.size()*sizeof(float),cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(dg,gamma.data(),gamma.size()*sizeof(float),cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(db,beta.data(),beta.size()*sizeof(float),cudaMemcpyHostToDevice));
  auto launch=[&]{transformer_ops::launch_residual_layernorm(dx,dr,dg,db,dy,dro,o.rows,o.cols);};
  float ms=gpu_ms(launch,o.warmup,o.iterations); double cpu=0; float err=0;
  if(o.check){auto t=std::chrono::steady_clock::now(); for(int r=0;r<o.rows;++r){double mean=0,sq=0; for(int c=0;c<o.cols;++c){float v=x[r*o.cols+c]+residual[r*o.cols+c];residual_out[r*o.cols+c]=v;mean+=v;}mean/=o.cols;for(int c=0;c<o.cols;++c){double d=residual_out[r*o.cols+c]-mean;sq+=d*d;}double inv=1/std::sqrt(sq/o.cols+1e-5);for(int c=0;c<o.cols;++c)ref[r*o.cols+c]=(residual_out[r*o.cols+c]-mean)*inv*gamma[c]+beta[c];}cpu=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-t).count();CUDA_CHECK(cudaMemcpy(out.data(),dy,out.size()*sizeof(float),cudaMemcpyDeviceToHost));err=max_error(out,ref);if(err>3e-4f)throw std::runtime_error("residual-layernorm correctness check failed");}
  report("residual-ln",o,ms,cpu,err); CUDA_CHECK(cudaFree(dx));CUDA_CHECK(cudaFree(dr));CUDA_CHECK(cudaFree(dg));CUDA_CHECK(cudaFree(db));CUDA_CHECK(cudaFree(dy));CUDA_CHECK(cudaFree(dro));
}

int main(int argc,char** argv){ try { auto o=parse(argc,argv); int devices=0; CUDA_CHECK(cudaGetDeviceCount(&devices)); if(!devices) throw std::runtime_error("no CUDA device found"); cudaDeviceProp p{}; CUDA_CHECK(cudaGetDeviceProperties(&p,0)); if(o.csv) std::cout<<"op,variant,m,n,k,rows,cols,gpu_ms,cpu_ms,max_error\n"; else std::cout<<"Device: "<<p.name<<"\n"; if(o.op=="all"||o.op=="matmul") run_matmul(o); if(o.op=="all"||o.op=="softmax") run_rowwise(o,false); if(o.op=="all"||o.op=="layernorm") run_rowwise(o,true); if(o.op=="all"||o.op=="residual-layernorm") run_residual_layernorm(o); return 0; } catch(const std::exception& e){ std::cerr<<"error: "<<e.what()<<'\n'; return 1; } }
