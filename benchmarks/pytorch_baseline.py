...
def main():
    ...
    d = "cuda"; x = torch.randn(a.rows, a.cols, device=d); residual = torch.randn_like(x)
    gamma = torch.randn(a.cols, device=d); beta = torch.randn(a.cols, device=d)
    ...
    cases = {"matmul": lambda: left @ right, "softmax": lambda: torch.softmax(x, dim=-1),
             ...
             "residual-ln": lambda: torch.nn.functional.layer_norm(x + residual, (a.cols,), gamma, beta)}
